...
#define MUNPA_MEMBER		2	//���Ŀ�
#define MUNPA_SENIOR		3	//���
#define MUNPA_VICE_MASTER	10	//�ι���
...
void CMunpaInfo::MakeToolTip(POSTYPE pos, WORD idx, char* str)
{
	...
	buff.Format("������ : %s(%d)", pInfo->ItemName, pInfo->ItemIdx);
	strcat(str, buff.GetBuffer(0));
	...
	if( GAMERESRCMNGR->IsOptionItem(pInfo->ItemIdx, m_pDocument->m_pWareHouse->WareHouse[pos].Durability) )
	{
		...
		buff.Format("\n�ɼǹ�ȣ : %d", m_pDocument->m_pWareHouse->WareHouse[pos].Durability);
		strcat(str, buff.GetBuffer(0));
	}
	else
	{
		...
		{
			buff.Format("\n��   �� : %d��", m_pDocument->m_pWareHouse->WareHouse[pos].Durability);
			strcat(str, buff.GetBuffer(0));
		}
	}
...
void CMunpaInfo::ButtonControlCommand(UINT nId)
{
	...
	{ // ���� â��
		...
		{
			if( MessageBox("������ �����Ͻðڽ��ϱ�?",NULL, MB_YESNO) == IDNO )
				return;
		}
...
void CMunpaInfo::EditControlCommand(UINT nId)
{
	...
	if( nId >= IDC_MUNPA_NAME && nId < IDC_MUNPA_LEVEL+1 )
	{
		wPos = (WORD)(nId - IDC_MUNPA_NAME);	// Tool �������� �迭�ε���
