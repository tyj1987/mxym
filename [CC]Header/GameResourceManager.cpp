...
CGameResourceManager::CGameResourceManager()
{
	...
	//2007. 12. 4. CBH - ��Ų ������ ����Ʈ ���̺� �ʱ�ȭ
	m_NomalClothesSkinTable.Initialize(100);
	//2008. 1. 16. CBH - �ڽ�Ƭ ��Ų ������ ����Ʈ ���̺� �ʱ�ȭ
...
CGameResourceManager::~CGameResourceManager()
{
	...
	RemoveNomalClothesSkinList();	// 2007. 12. 4. CBH - ��Ų ������ ��� ����Ʈ ����
	RemoveCostumeSkinList();	// 2008. 1. 16. CBH - �ڽ�Ƭ ��Ų ������ ��� ����Ʈ ����
...
BOOL CGameResourceManager::LoadTitanList()
{
	...
	while(!file.IsEOF())
	{
		...
		for( int i = 0; i < MAX_TITAN_GRADE; ++i )
		{
			...
			pGrade->dwRecallSpellTime = file.GetDword(); //2007. 11. 19. CBH - Ÿ��ź ��ȯ �ֹ� �ð� �߰�
...
//		file.GetString(pList->Name);						// �����̸�
//		file.GetString(pList->EngName);						// �����̸�
...
//		file.GetString(pList->ChxName);
		...
		pList->AttackPhysicalMin		= file.GetWord();// ���� �ּҹ��� ���ݷ�	
		pList->AttackPhysicalMax		= file.GetWord();// ���� �ִ빰�� ���ݷ�	
		pList->CriticalPercent			= file.GetWord();// ũ��Ƽ�� �ۼ�Ʈ
		pList->PhysicalDefense			= file.GetWord();// ���� ����	
		pList->AttribResist.SetElement_Val(ATTR_FIRE,file.GetFloat());// �Ӽ����׷�(ȭ)% 	
		pList->AttribResist.SetElement_Val(ATTR_WATER,file.GetFloat());// �Ӽ����׷�(��)% 	
		pList->AttribResist.SetElement_Val(ATTR_TREE,file.GetFloat());// �Ӽ����׷�(��)% 	
		pList->AttribResist.SetElement_Val(ATTR_IRON,file.GetFloat());// �Ӽ����׷�(��)% 	
		pList->AttribResist.SetElement_Val(ATTR_EARTH,file.GetFloat());// �Ӽ����׷�(��)% 

		pList->WalkMove					= file.GetWord();// ���Ͱȱ��̵���	
		pList->RunMove					= file.GetWord();// ���Ͷٱ��̵���	
		pList->RunawayMove				= file.GetWord();// ���͵����̵���
		...
		pList->MinDropMoney				= file.GetDword();// �ּҵ� �׼�	
		pList->MaxDropMoney				= file.GetDword();// �ִ� �� �׼�	
		ASSERT(pList->MaxDropMoney >= pList->MinDropMoney);
		pList->DropRate[eMONEY]			= file.GetWord();// �� Ȯ��(%)	
		pList->DropRate[eITEM1]			= file.GetWord();// ������ Ȯ��(%)	
		pList->DropRate[eITEM2]			= file.GetWord();// ������ Ȯ��(%)	
		pList->DropRate[eITEM3]			= file.GetWord();// ������ Ȯ��(%)	
		pList->DropRate[eNOITEM]			= file.GetWord();// ���� Ȯ��(%)	
...
#endif
		pList->ItemListNum1				= file.GetWord();// �ش�����۸����ǥ��ȣ	
		pList->ItemListNum2				= file.GetWord();// �ش�����۸����ǥ��ȣ	
		pList->ItemListNum3				= file.GetWord();// �ش�����۸����ǥ��ȣ	

		pList->MaxDropItemNum			= file.GetByte();// �ִ������ ����
...
BOOL CGameResourceManager::LoadExpPoint()
{
	...
	while(1)
	{
		...
		if(file.IsEOF())
			break;
		file.GetWord();												// ����
		m_CharacterExpPoint[idx] = file.GetExpPoint64();				// ����ġ
		idx++;
	}
	...
	while(1)
	{
		...
		else if(strcmp(value,"*WINDOWMODE") == 0)
		{
			if( file.GetBool() )
				...
				m_GameDesc.dispInfo.dispType = /*FULLSCREEN_WITH_FLIP;//*/FULLSCREEN_WITH_BLT;	//FLIP�� Alt_Tab�� �ȵȴ�.
