...
void CPlayer::InitClearData()
{
	...
	memset(&m_setItemStats,0,sizeof(m_setItemStats));		// 2007. 6. 12. CBH - ��Ʈ������ �ɷ�ġ ����ü �ʱ�ȭ �߰�.
...
*/
	...
	m_FullMoonEventHCt = 0;
	m_fFullMoonEventTime = 0.f;
	m_dwHpMpBitFlag = 0x00;	// magi82	����ġ���� ����ȸ�� ���� �÷��� �ʱ�ȭ
...
DWORD CPlayer::GetGravity()
{
	...
	{
//		if( pObject->GetLastAttackPlayer() != this )//���� ������ ���Ͱ� �ƴѰ��
...
void CPlayer::SetAddMsg(char* pAddMsg,WORD* pMsgLen,DWORD dwReceiverID,BOOL bLogin)
{
	...
	GetSendMoveInfo(&pMsg->MoveInfo,&pMsg->AddableInfo,TRUE);	//SW071129 ���� �߰�
	...
	if( GetBattle()->GetBattleKind() == eBATTLE_KIND_VIMUSTREET )	//����n��o��iA�ϡ�AI ��O����
	{
...
*/
// LBS 03.11.04 flagA����������� ��o��������C��ic����a��?I ����E����a�����ˡ��������� (CoAc) 0:����E����a, 1:��ic����a, 
// 3:(DB��u��ACI����i, A������OoAI��u�ϡ̡�����c��?��� ��uE�ˡ���c�ˡ�e��u���� �ˡ�������u��uAo�ˡ�| ��Oc��?iAo ��uE�ˡ�A�ˡ�U.)
...
void CPlayer::SetLife(DWORD val,BOOL bSendMsg) 
{
	...
	if(m_HeroCharacterInfo.Life != val)	// ��i��I��oIAo AI���������?i��?���ˡ�A ��?����Aa��?�ˢ硧u���� ��i��IA�����ˡ�U.
	{
		if(bSendMsg == TRUE)
		{
			// To A������OoAI��u�ϡ̡�����c -------------------------------
...
void CPlayer::SetShield(DWORD val,BOOL bSendMsg) 
{
	...
	if(bSendMsg == TRUE)	// ��i��I��oIAo AI���������?i��?���ˡ�A ��?����Aa��?�ˢ硧u���� ��i��IA�����ˡ�U.
	{
		// To A������OoAI��u�ϡ̡�����c -------------------------------
		MSG_INT msg;
		msg.Category = MP_CHAR;
...
{	
	// A������OoAI��u�ϡ̡�����c��?�ˢ硧u���� �����ˡ���?��IAO�ˡ�A����O ��u����ˡ�A��i��I ��?O A�ϡ��������C�ϡ�;����i�ϩ�����... hs
/*
		CParty* pParty = PARTYMGR->GetParty(PartyID);
		if(pParty == NULL)
			return;
...
/*****************************************************************/
/* 1. AE����a��?��� A�ϩ��ˡ�?AI������� ��iy��u������iC��uu;��O���� from DBResultQuery
/* 2. SetPlayerExpPoint()��?�ˢ硧u���� AO�ˡ�e �������CeA���ˡ�| �ϩ�N��ui ��u��o; ��O���� call��iE
/*
/*
/*****************************************************************/
...
{
	if(level >= MAX_CHARACTER_LEVEL_NUM ) //MAX_PLAYERLEVEL_NUM ���ｫ�ȼ�����ȥ��
	{
		ASSERT(0);
		return;
	}
...
//		LEVELTYPE aftermaxLevel = GetMaxLevel();
		...
		// magi82 LevelUp ����ȭ ���� -> �������� ������ �ְ������� �����ؾ߸� ���� �÷��׸� �ʱ�ȭ(2���� �ٿ��� 1������ ������ �ʱ�ȭ�Ǹ� �ȵǹǷ� �۾���)
...
//		{
			...
			// ������ �����Ƿ� flag�� ��� �ʱ�ȭ(�ٽ� 20% 40% 60% 80% �϶� ���ݰ� ����ȸ���� ����ϴϱ�..)
//			m_HeroInfo.ExpFlag = 0x00;
//			SetOnBit(&m_HeroInfo.ExpFlag, 4);	// magi82 LevelUp ����ȭ ����
...
//			m_dwHpMpBitFlag = 0x00;
		...
		// Object AI������I������c��?��� �ˡ�eCN A��uA��I�ˡ�| CN�ˡ�U.
...
{	
	...
	if( !(CHANNELSYSTEM->IsBattleChannel(GetChannelID()) && IsDieForBattleChannel()) )	//SW080515 ä�κ� ����
...
//				else				minusExp = (EXPTYPE)(pointForLevel * 0.03);
	...
	m_bDieForBattleChannel = FALSE;	//SW080515 ä�κ� ����
	m_bDieForGFW = FALSE;
	m_bDieInSpecialMap = FALSE;
}
...
void CPlayer::DoDie(CObject* pAttacker)
{
	...
	if(pAttacker->GetObjectKind() == eObjectKind_Player)
	{
		...
		if(pBattle->GetBattleKind() == eBATTLE_KIND_NONE || pAttacker->GetBattleID() != GetBattleID() )
		{
			...
			if(pPet)
			{
				...
				if(wMapNum != Tournament && wMapNum != QUESTMAPNUM1 && wMapNum != QUESTMAPNUM2)		//ȭ��/�ظ� ���� �� 73
				{
					...
					if( (TRUE == PARTYWARMGR->IsEnemy(this, pAttackPlayer) && FALSE == pAttackPlayer->IsPKMode())		//������ ����̸� ��� ���°� �ƴϸ�
						|| TRUE == GUILDFIELDWARMGR->IsEnemy(this, pAttackPlayer) && FALSE == pAttackPlayer->IsPKMode() )	//������ ����̸� ��� ���°� �ƴϸ�
					{
						//Do nothing
					}
					else
	...
	if(IsMussangMode())
	{
		...
		STATSMGR->CalcItemStats(this);	//���� ���� �ʱ�ȭ.
		SendMussangEnd();
	}
...
/* ��i����; �ϩ�O; A�����������CN �������������IAI AO�ˡ�AAo�ˡ�| ��o?�ˡ�A�ˡ�U.*/
...
{
	//C��I������o; ��ic�ˡ�e ��i���� �����ˡ�����u �ˡ�������o����A�ˢ������ �ˡ�A��ui�ϩ����ϡˡ�U.
...
BOOL CPlayer::SetQuestState(DWORD QuestIdx, QSTATETYPE value)
{
	...
	{
//		char buff[256] = {0,};
//		sprintf(buff, "��?a����a�ˡ�A Au��o����������c������� xAc ��uECN�ˡ�U�ϩ��ϡ� CI��?��I������I ��oy��u��c��?�ˢ���O ��uE����AAa��?���� [QUEST ID : %d]", QuestIdx);
//		ASSERTMSG(0, buff);
		return FALSE;
	}
...
//		(m_initState & PLAYERINITSTATE_GMCONTROL_INFO) &&
		...
		(m_initState & PLAYERINITSTATE_SHOPITEM_USEDINFO) )
	{
		...
		//SW060718 ���̵� ���� ���� �� ��ġ�̵�. SetInitedGrid() ����..
		GetPetManager()->CheckSummonningPet();

		//GetTitanManager()->CheckRidingTitan();	//Inited ���ķ�..

		// A�ϩ��ˡ�?d�����ˡ� + ��o��i�������d�����ˡ� + ��u����AIAUd�����ˡ�������� �ˡ��ϡ̡�iI ��uA����A��i����8��oC����I CA������oA��I��uA����AEA Send CN�ˡ�U
		CalcState(); //�ˡ�OAu ����e��ie��uoA����ie; ����e��ieCN�ˡ�U. LBS 03.11.20
...
/*
		...
		msg.OptionNum = 0;	//�ʱ�ȭ ��
		WORD OptionNum = 0;
		ITEM_OPTION_INFO OptionInfo[MAX_ITEM_OPTION_NUM+SLOT_PYOGUK_NUM]; //â���� ������ ������2006.4.8
		m_ItemOptionList.SetPositionHead();
		ITEM_OPTION_INFO * pOInfo = NULL;
		while(pOInfo = m_ItemOptionList.GetData())
		{
		...
		//����ɼ�
		msg.RareOptionNum = 0;	//�ʱ�ȭ ��
		OptionNum = 0;
		ITEM_RARE_OPTION_INFO RareOptionInfo[MAX_ITEM_OPTION_NUM+SLOT_PYOGUK_NUM]; //â���� ������ ������2006.4.8
		m_ItemRareOptionList.SetPositionHead();
		ITEM_RARE_OPTION_INFO* pRInfo = NULL;
		while(pRInfo = m_ItemRareOptionList.GetData())
		{
...
//		return FALSE;
	...
	if( LOOTINGMGR->IsLootedPlayer(GetID()) )	//PK����c����A; �ˡ�cCI�ˡ�A A�ϡ�AI�������?
...
int CPlayer::PKModeOn()
{
	...
	if( IsVimuing() ) return ePKCODE_VIMUING;		//����n��o��iA�ϡ�AI�ˡ�e ��uE��iE�ˡ�U
...
void CPlayer::StateProcess()
{
	...
	// Play Time
	// ��iA��CC ��cCA���A��A ��u��I������a��A��I.. ��A AOA����I��i ���ϡ�a���� CIAU.. ������������ RaMa
...
{	
	...
	{
		// ��o��i����a�ˡ�| ��oU�ϡ�U�ˡ�e Ao�������: ��ic��OoA����ˡ�U.
...
//KES PKLOOTING
	...
	if(pAttacker->GetObjectKind() == eObjectKind_Player)
	{
		...
		if(pBattle->GetBattleKind() == eBATTLE_KIND_NONE || pAttacker->GetBattleID() != GetBattleID() )
		{
			...
			{
				//PKLooting!
				//IsLootingSituation����������A ��n����A��AIAo ��E��c ��ECN��U. ������������ ���ϡ�a��U��� ��O��u��U.
				if( LOOTINGMGR->IsLootingSituation( this, pAttacker ) )
				{
					LOOTINGMGR->CreateLootingRoom( this, (CPlayer*)pAttacker );
				}
...
void CPlayer::ApplyTitanWearItem()
{
	...
	if(GetInited())
	{
		...
		m_TitanManager.SendTitanPartsChange();	//Ÿ��ź ������ ���� ���� ���氡��.
	}

	//2007. 10. 5. CBH - Ÿ��ź ��� ����� Ÿ��ź EP��ġ ����
	if(InTitan() == TRUE)
	{
