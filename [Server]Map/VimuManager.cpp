...
void CVimuManager::UserLogOut( CPlayer* pPlayer )
{
	if( pPlayer->IsVimuing() )		// û ó
	{
		...
		if( pPlayer->GetState() == eObjectState_BattleReady )	//û
		{
			MSGBASE CancelMsg;
	...
	case MP_BATTLE_VIMU_APPLY_SYN:
		{
			...
			pPlayer1->SetVimuing( TRUE );		//񹫸Ŵ.. ÷̾Ѵ? //confirm
	...
	case MP_BATTLE_VIMU_ACCEPT_SYN:
		{
			...
			float fDist = CalcDistanceXZ( &vPosApplyer, &vPosAccepter );	//confirm ȭ
...
//			if( pPlayer1->IsVimuing() == TRUE )	//̹ ۵Ǿ.
