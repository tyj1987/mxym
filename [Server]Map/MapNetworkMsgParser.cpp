	...
	case MP_CHAR_PLAYTIME_SYN:
		{
			...
			else
			{
				...
				// Assert
				char buf[256] = { 0, };
				sprintf(buf, "�÷��̽ð��� ���������� ������ϴ�. player:%d Server:%d ecost:%d allowance:%d",
...
void MP_USERCONNMsgParser(DWORD dwConnectionIndex, char* pMsg, DWORD dwLength)
{
	...
	case MP_USERCONN_GAMEIN_OTHERMAP_SYN:	//from agent. �ٸ������� ���µ�..Ȥ�ö� �̸ʿ��� ������ �����ַ���
		{
			MSG_DWORD* pmsg = (MSG_DWORD*)pMsg;
...
//			QuestItemload(pmsg->dwObjectID);
		...
		break;
	case MP_USERCONN_GAMEINPOS_SYN:		// Ŭ���̾�Ʈ���� ������
		{
			MSG_DWORD4* pmsg = (MSG_DWORD4*)pMsg;
...
//			QuestItemload(pmsg->dwObjectID);
	...
	case MP_USERCONN_GAMEOUT_SYN:	//������Ʈ�� ĳ���Ͱ� �ִµ� �� ĳ���� ������ �Դ�. Ȥ�� �ʿ��� ���� �ִ��� ���°Ŵ�.
		{
			MSG_DWORD* pmsg = (MSG_DWORD*)pMsg;
	...
	case MP_USERCONN_DISCONNECTED:
		{
			...
			if( !pPlayer->IsNormalExit() )
			{
				...
				//AUTONOTE
				if( pPlayer->GetAutoAskPlayerIdx() )				//---����Ű����� ĳ����
				{
...
//			if( pPlayer->IsPKMode() || LOOTINGMGR->IsLootedPlayer( pPlayer->GetID() ) || eObjectState_Die == pPlayer->GetState() )
			if( pPlayer->IsPKMode() ||
				...
				pPlayer->GetAutoAskPlayerIdx() )					// autonote �Ű����̸�...
			{
				MSGBASE msgNack;
	...
	case MP_MOVE_STOP:
		{
			...
			if(CCharMove::CheckVaildStop(pPlayer,gCurTime,&pos) == TRUE)
				CCharMove::EndMove(pPlayer,gCurTime,&pos);	// �������̸� EndMove
...
//			PARTYMGR->AddParty(pmsg->dwData2,pmsg->dwObjectID,pmsg->dwData1); //include DB
	...
	case MP_PARTY_NOTIFYCREATE_TO_MAPSERVER:
		...
		break;
		*/
	case MP_PARTY_NOTIFYCREATE_TO_MAPSERVER:	//2008. 6. 3. CBH - ����
		{
	...
	case MP_PARTY_MASTERTOREQUEST_SYN:
		...
		break;		
	case MP_PARTY_REQUEST_CONSENT_SYN:	//��Ƽ ���� ó��
		{
			MSG_DWORD2* pmsg = (MSG_DWORD2*)pMsg;
	...
	case MP_PARTY_NOTIFY_INFO:	//��Ƽ ���� ����
		...
		break;
	case MP_PARTY_MATCHING_INFO:	//��Ƽ��Ī ����Ʈ ���� ��û
		{
			MSG_WORD* pmsg = (MSG_WORD*)pMsg;
	...
	case MP_QUEST_CHANGESTATE:
		{
			...
			{
//				char buff[256] = {0,};
//				sprintf(buff, "����߸� �������� ������Ź����� [Client ID : %d]", pmsg->dwObjectID);
//				ASSERTMSG(0, buff);
				return;
			}		
