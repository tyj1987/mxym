...
CGameIn::CGameIn()
{
	...
	// magi82 - Titan(070119) ////////////////////////////////////////////////
	m_pTitanMixDlg			= NULL;	
	m_pTitanRepairDlg		= NULL;	//2007. 9. 11. CBH - Ÿ��ź ����â
	m_pTitanRecallDlg		= NULL;	//2007. 9. 13. CBH - Ÿ��ź ��ȯ ������
	...
	//m_pTitanMugongMixDlg	= NULL;		// magi82 - Titan(070611) Ÿ��ź ������ȯ �ּ�ó��
...
// LYJ 051017 ���Գ����� �߰�
	...
	m_pSkillPointResetDlg	= NULL;	// magi82(42) - �������� �߰�(����ġ �ʱ�ȭ)	
	...
	m_pStallFindDlg = NULL; // ������ �˻� �߰� by Stiner(8)
}
...
//	GAMERESRCMNGR->LoadSMonsterList();
	...
	ITEMMGR->LoadSetItemOption();			//2007. 6. 14. CBH - ��Ʈ������ ������ �ε� �߰�
...
//��UAaA�� A����A����A��A��C������. ��AA�ϡ�IinitAIEA E��Aa.
...
// LYJ 051017 ���Գ����� �߰�
	...
	m_pUniqueItemMixProgressBarDlg = NULL;		//2007. 10. 22. CBH - ����ũ ������ ���� ó�� �� ������
	m_pTitanMixProgressBarDlg = NULL;		//2007. 10. 22. CBH - Ÿ��ź ���� ó�� �� ������
	m_pTitanPartsProgressBarDlg = NULL;		//2007. 10. 23. CBH - Ÿ��ź ���� ���� ó�� �� ������
...
//	MINIMAP->Release();
	...
	if(m_GameInInitKind != eGameInInitKind_SuryunEnter && m_GameInInitKind != eGameInInitKind_EventMapEnter)
		USERINFOMGR->SaveUserInfo( eUIK_USERSTATE | eUIK_INTERFACE );	//HERO�Ƣ� ������iAo��a Au����.
...
//RESRCMGR->ReleaseResource();
	...
	g_pExecutive->UnloadAllPreLoadedGXObject(0);	//NULLA���Ƣ�
...
//	}
	...
	//if( m_iMilliSecend >= 10000 ) //1�� = 1000
	//{
...
//		STATUSICONDLG->Render();	//windowmanager���� ���ش�(����������)
		BATTLESYSTEM->Render();
		WEATHERMGR->Render();
		WINDOWMGR->Render();
...
//					LOGFILE("AddGarbageObject\t%u",pmsg->dwData);
					...
					if(pObject->GetObjectKind() == eObjectKind_Player)
					{
						...
						if(pPet)
						{
							pPet->SetMaster(NULL);	//�� ������Ʈ�� ���������� ������ �־�� ���� GridIn �� ����� �ൿ.
						}*/
...
//					//virtual LEVELTYPE GetLevel(){ return m_pSInfo->Level; } CMonster.h �߰����
			...
			case MP_USERCONN_CHARACTER_ADD:
				{
					...
					// Add Player�� �Ϸ�� �������� PostAddPlayer()�� ȣ��˴ϴ�. 
					// Effect���� ���� �� Player model �� �̸� �ҷ������� �뿡 
					// ��¿ �� ���� �帧�� �ٲ���ϴ�. ������ �ֽʽÿ�. 
					...
					{
						//pPet
						//pPet->SetPetMasterName(pPlayer->GetID());	//���� ����
						pPlayer->SetPet(pPet);
...
//					CPet* pPet = OBJECTMGR->AddPet(&pmsg->BaseObjectInfo, &moveInfo, &pmsg->TotalInfo, pmsg->MasterID);
					...
					if(pPlayer)
					{
						...
						if(HEROID == pmsg->MasterID)
						{
							...
							//PETMGR->OpenPetStateDlg();
							PETMGR->SetPetStateDlgInfo(pPet);	//�����â(ū) ���� ����.
							PETMGR->SetPetStateMiniDlgInfo(pPet);	//�����â(��) ���� ����.
					...
					pPet->SetMasterID(pmsg->MasterID);	//������ ���� ���� �׸��峻�� ���� ������ ��� ���� ����ID ���� ������ �ִ´�. ���� �߰��� ����.
...
//					PEACEWARMGR->Peace_WarMode(HERO, pmsg->wData1);
					...
					// magi82(37) �� �Ӽ� ������ GetMapName( MAP->GetMapNum() ) ����
#ifdef TAIWAN_LOCAL					
...
//----------------------------------------------------------------------------------------------
			case MP_USERCONN_EVENTITEM_USE:
				{
					...
					if( pmsg->dwData != 0 )
						...
						CHATMGR->AddMsg( CTC_SYSMSG, "������ ���� 50���� ��� ����Ͽ����ϴ�." );	// �̺�Ʈ ���� �ӽ÷� ����...
				}
				break;
			...
			case MP_USERCONN_FLAGNPC_ONOFF:
				...
				break;
			case MP_USERCONN_CHANGEMAP_CHANNELINFO_ACK:	//2008. 5. 8. CBH - �̵��� �� ä�������� �޴� �������� �߰�
				{
			...
			case MP_USERCONN_CHANGEMAP_CHANNELINFO_NACK:	//2008. 5. 8. CBH - �̵��� �� ä�������� �޴� �������� �߰�
				{
					...
					case 82:
						{
							...
							if( nLen > 0 && strcmp( pNpc->GetObjectName() + nLen, "���±�" ) == 0 )
							{
							...
							if( nLen > 0 && strncmp( pNpc->GetObjectName(), "ͨ��", 4 ) == 0 )
							{
							...
							if(pMCInfo == NULL) ASSERTMSG(0,"�ùٸ� ������ ���� �� �����ϴ�.");
			...
			case MP_ITEMLIMIT_FULL_TO_CLIENT:
				{
					...
					if( pInfo )
                        CHATMGR->AddMsg( CTC_ALERT_YELLOW, "%s ������ �������� ���̻� ���� �� �����ϴ�.", pInfo->ItemName );
					else
						CHATMGR->AddMsg( CTC_ALERT_YELLOW, "������[%d] �������� ���̻� ���� �� �����ϴ�.", pmsg->dwData );
				}
				break;
			}
		}
