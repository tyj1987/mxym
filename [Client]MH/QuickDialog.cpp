...
{
	int i;  // 修复C2065: 在函数作用域声明i
	cButton * win;  // 修复C2065: 在函数作用域声明win
	if(!m_bActive) return;
...
BOOL CQuickDialog::GetAbsPositionForXYRef(LONG x, LONG y, POSTYPE & abs_pos)
{
	...
	{
		abs_pos = position+TP_QUICK_START+TABCELL_QUICK_NUM*m_selectedPage;								// ���� ��ġ�� �ٲ�
//		abs_pos = position+QUICK_STARTPOSITION+MAX_QUICKITEMPERSHEET_NUM*m_selectedPage;								// ���� ��ġ�� �ٲ�
		return TRUE;
	}
	return FALSE;
}
...
//==========================
			...
			if( pInfo->ItemIdx == 53102 || pInfo->ItemIdx == 53232 || pInfo->ItemIdx == 53234 )
			{
				...
				wsprintf(ToolTipText, "%s %d", CHATMGR->GetChatMsg(270), plus);	//������ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
				wsprintf(ToolTipText, "%s %d", CHATMGR->GetChatMsg(271), plus);	//����ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
				wsprintf(ToolTipText, "%s %d", CHATMGR->GetChatMsg(181), plus);	//ȣ�Ű���ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
			}
			else if( pInfo->ItemIdx == 53031 || pInfo->ItemIdx == 53094 )
			{
				...
				|| pInfo->ItemIdx == 53103 )
			{
				wsprintf(ToolTipText, "%s %d%%", CHATMGR->GetChatMsg(270), 100);	//������ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
				wsprintf(ToolTipText, "%s %d%%", CHATMGR->GetChatMsg(271), 100);	//����ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
				wsprintf(ToolTipText, "%s %d%%", CHATMGR->GetChatMsg(181), 100);	//ȣ�Ű���ȸ��
				item->AddToolTipLine( ToolTipText, TTTC_EXTRAATTR );
			}
#endif
		}
		...
		if( wCommSkillKind == SKILLKIND_TITAN )
		{
			if( wCommSkillWeaponType != WP_WITHOUT )	// ������ ���Ⱑ ����
			{
				if( wHeroWeaponType != wTitanWeaponType )
				{
					item->SetImageRGB( ICONCLR_DISABLE );
					bUsable = FALSE;
					//item->AddToolTipLine( "magi82 - Ÿ��ź�� ĳ������ ���Ⱑ �ٸ�", TTTC_LIMIT );
				}
			}
		}
